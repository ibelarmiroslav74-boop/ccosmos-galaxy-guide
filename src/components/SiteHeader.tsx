...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 px-3 pt-3">
      <div className="panel rounded-full mx-auto max-w-6xl relative">
        <div className="h-12 px-3 sm:px-4 flex items-center gap-3 sm:gap-4 text-[13px]">
          {searchOpen ? (
            <SearchBar onClose={() => setSearchOpen(false)} />
          ) : (
            <>
              <Link to="/" className="font-medium tracking-tight shrink-0 pl-1">
              ...
              <div className="ml-auto flex items-center gap-1.5 sm:gap-2">
                <button
                  ...
                  className="h-8 px-2.5 sm:px-3 flex items-center gap-2 rounded-full text-muted-foreground hover:text-foreground hover:bg-white/[0.06] transition-colors"
                >
                  ...
                  <kbd className="hidden md:inline-block text-[9px] uppercase tracking-widest border border-hairline rounded px-1 py-px">
                    ⌘K
                  </kbd>
                </button>
